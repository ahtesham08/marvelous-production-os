...
import { BRAINSTORMING_DECISIONS, buildBrainstormingReport } from "@/lib/sharedConstants";
...
export function LiveMeetingClient({ session, titles, canDecide }: { session: BrainstormingSession; titles: BrainstormingTitle[]; canDecide: boolean }) {
  ...
  const report = buildBrainstormingReport(session, titles);
              ...
              <article key={title.id} className="rounded-lg border border-black/10 bg-[#f6f4ee] p-4">
                <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-black/60">{title.channel || "MV N"} | {title.priority || "Normal"} | Writer: {title.suggested_writer || "Optional"}</p>
                  </div>
                  ...
                  <div className="mt-4 grid gap-2 sm:grid-cols-3">
                    {BRAINSTORMING_DECISIONS.map((decision) => (
                      <button
                        ...
                        onClick={() => {
                          const reason = window.prompt(`Reason for: ${decision}`, title.decision_reason ?? "");
                          patchTitle(title.id, { action: "decision", decision, reason });
                        }}
